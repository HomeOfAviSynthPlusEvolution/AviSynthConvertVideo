...
全部已测结果见 [PERFORMANCE.md](PERFORMANCE.md)：**522 个完整滤镜场景**、**592 行内核比较**及 **108 行长核补充记录**，补充记录与主审计有重叠。下表覆盖全部已测类别。数值为**新 / 上游耗时比**，小于 1 表示更快；中位数是各场景比例的中位数，不是总耗时加速比。极低工作量场景不参与汇总。
...
性能取决于格式、核宽度、CPU 和编译器。更新后的 Floyd–Steinberg 完整滤镜耗时比中位数接近上游，部分场景仍较慢。重采样结果随格式、核宽度和 CPU 目标而变化。
